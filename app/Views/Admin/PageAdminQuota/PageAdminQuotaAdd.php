...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
        <div>
            ...
            <p class="text-muted mb-0 small">กำหนดประเภทโควต้า ระดับชั้นที่เปิดรับ และเลือกหลักสูตรที่เกี่ยวข้อง</p>
        </div>
    ...
    <form action="<?= site_url('skjadmin/quotas/create') ?>" method="post" id="quotaForm">
        <?= csrf_field() ?>
            ...
            <div class="col-lg-6">
                ...
                <div class="card border-0 rounded-4 shadow-sm bg-white mb-4 overflow-hidden">
                    <div class="card-header bg-white border-bottom py-3 px-4 d-flex align-items-center gap-2">
                        ...
                        <div>
                            <h5 class="mb-0 fw-bold text-dark">ข้อมูลทั่วไปของโควต้า</h5>
                            <small class="text-muted">กำหนดชื่อและรหัสอ้างอิงของโควต้า</small>
                        </div>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <div class="mb-3">
                            <label class="form-label fw-bold text-dark small d-flex align-items-center gap-1 mb-2">
                                <i class="bx bx-bulb text-warning fs-5"></i> แม่แบบโควตายอดนิยม (คลิกเพื่อเลือกใช้อัตโนมัติ):
                            </label>
                            <div class="d-flex flex-wrap gap-2">
                                <button type="button" class="template-chip" onclick="applyTemplate('quota_sport', 'โควตานักกีฬา (ช้างเผือก / กีฬาเด่น)', ['1', '4'])">
                                    🏃 โควตานักกีฬา
                                </button>
                                <button type="button" class="template-chip" onclick="applyTemplate('quota_academic', 'โควตานักเรียนเรียนดี (ผลการเรียนยอดเยี่ยม)', ['1', '4'])">
                                    🎓 โควตานักเรียนเรียนดี
                                </button>
                                <button type="button" class="template-chip" onclick="applyTemplate('quota_talent', 'โควตาความสามารถพิเศษ (ดนตรี, ศิลปะ, นาฏศิลป์)', ['1', '4'])">
                                    🎭 โควตาความสามารถพิเศษ
                                </button>
                                <button type="button" class="template-chip" onclick="applyTemplate('quota_internal_m3', 'โควตานักเรียนเดิม ม.3 ขึ้น ม.4', ['4'])">
                                    🏫 โควตานักเรียน ม.3 เดิม
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control rounded-end-3 font-monospace border-start-0" id="quota_key" name="quota_key" placeholder="เช่น quota_sport, quota_academic" required>
                            </div>
                            <small class="text-muted d-block mt-1">
                                <i class="bx bx-info-circle me-1"></i> ภาษาอังกฤษพิมพ์เล็ก ไม่มีช่องว่าง (ใช้เป็นรหัสอ้างอิงในระบบ)
                            </small>
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control rounded-3" id="quota_explain" name="quota_explain" rows="3" placeholder="ระบุชื่อโควต้า เช่น โควตานักเรียนเรียนดี, โควตานักกีฬา" required></textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="d-flex align-items-center justify-content-between p-3 rounded-3" style="border: 1.5px solid #e2e8f0; background: #ffffff;">
                                <div>
                                    <h6 class="mb-0 fw-bold text-dark">เปิดรับสมัครทันที</h6>
                                ...
                                <div class="form-check form-switch m-0">
                                    ...
                                    <input class="form-check-input" type="checkbox" id="quota_status_switch" name="quota_status" value="on" checked style="width: 3rem; height: 1.5rem; cursor: pointer;">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body p-4">
                        ...
                        <div class="row g-2">
                            <?php foreach ($levels as $val => $info): ?>
                                <div class="col-sm-6">
                                    <div class="level-check-card <?= in_array($val, ['1']) ? 'checked' : '' ?>" onclick="toggleLevelCard('level_<?= $val ?>', this)">
                                        <input class="form-check-input" type="checkbox" name="quota_level[]" value="<?= $val ?>" id="level_<?= $val ?>" <?= in_array($val, ['1']) ? 'checked' : '' ?> onclick="event.stopPropagation(); syncLevelCardStyle(this);">
                            ...
                            <div class="row g-2" id="m1CourseContainer">
                                <?php if (!empty($m1Courses)): ?>
                                    <?php foreach ($m1Courses as $c): ?>
                                        <div class="col-12">
                                            <div class="course-item-box" onclick="toggleCourseBox('course_<?= $c['course_id'] ?>', this)">
                                                <div class="d-flex align-items-center gap-2">
                                                    <input class="form-check-input course-m1 m-0" type="checkbox" name="quota_course[]" value="<?= $c['course_id'] ?>" id="course_<?= $c['course_id'] ?>" onclick="event.stopPropagation(); syncCourseBoxStyle(this);">
                                ...
                                <?php if (!empty($m4Courses)): ?>
                                    <?php foreach ($m4Courses as $c): ?>
                                        <div class="col-12">
                                            <div class="course-item-box" onclick="toggleCourseBox('course_<?= $c['course_id'] ?>', this)">
                                                <div class="d-flex align-items-center gap-2">
                                                    <input class="form-check-input course-m4 m-0" type="checkbox" name="quota_course[]" value="<?= $c['course_id'] ?>" id="course_<?= $c['course_id'] ?>" onclick="event.stopPropagation(); syncCourseBoxStyle(this);">
                                                    <div class="flex-grow-1">
        ...
        <div class="card border-0 rounded-4 shadow-sm bg-white mt-3 p-3">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-2">
                <span class="text-muted small">
                    <i class="bx bx-check-circle text-success me-1"></i> ตรวจสอบข้อมูลระดับชั้นและหลักสูตรให้ถูกต้องก่อนบันทึก
                </span>
                <div class="d-flex gap-2">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-5 shadow-sm fw-bold">
                        <i class="bx bx-save me-1"></i> บันทึกโควต้าใหม่
                    </button>
                </div>
            </div>
        </div>
...
<script>
    // Quick Template Auto-filler
    function applyTemplate(key, explain, levels) {
        document.getElementById('quota_key').value = key;
        document.getElementById('quota_explain').value = explain;
        
        // Reset all levels
        document.querySelectorAll('input[name="quota_level[]"]').forEach(cb => {
            cb.checked = levels.includes(cb.value);
            syncLevelCardStyle(cb);
        });
