...
class UserAuth extends \App\Controllers\BaseController
{
    ...
    {
        helper(['form', 'url']); // Ensure form helper is loaded
        ...
        if ($this->request->getMethod() === 'post') {
            ...
            if (! $this->validate($rules, $errors)) {
                ...
                $user = $model->where('pers_username', $this->request->getVar('username'))
                              ->first();

                if ($user) {
                    // Check if user has permission in tb_admin_rloes
                    $db = \Config\Database::connect();
                    $role = $db->table('tb_admin_rloes')->where('admin_rloes_userid', $user['pers_id'])->countAllResults();
                    ...
                    if ($role > 0) {
                        ...
                        $data['validation']->setError('username', 'คุณไม่มีสิทธิ์เข้าใช้งานระบบนี้');
                    }
                } else {
                    $data['validation'] = \Config\Services::validation();
                    $data['validation']->setError('username', 'ชื่อผู้ใช้ไม่ถูกต้อง');
                }
            }
        }
       ...
       // $data['checkOnOff'] = $this->db->table('tb_register_onoff')->select('*')->get()->getResult();
        ...
        return view('admin/PageAdminAuth/AdminLogin', $data);
    }
    ...
    {
        $session = session();
        $DB_General = \Config\Database::connect(); // Default database
        $DBrloes = $DB_General->table('tb_admin_rloes'); // Reverted: use default DB for rloes as per user
        $DB_Personnel = \Config\Database::connect('personnel'); // Personnel database
        $DBPers = $DB_Personnel->table('tb_personnel');     
        
        // This is from ConLogin - it assumes a 'return_to' in the request,
        // but it's usually handled by the redirect_uri in the Google config.
        // For now, I'll set a default return if no session return is found.
        if($this->request->getVar("return_to")){
             session()->set('Return', $this->request->getVar("return_to"));
        } else if (!session()->has('Return')) {
            session()->set('Return', base_url('admin/documents')); // Default admin dashboard
        }
        ...
        if($this->request->getVar("code")){
            ...
            if(!isset($token['error'])){
                ...
                if($CheckEmail){
                    ...
                    return redirect()->to(session()->get('Return'));
    ...
    {
        $db = \Config\Database::connect();
        $DBrloes = $db->table('tb_admin_rloes');
